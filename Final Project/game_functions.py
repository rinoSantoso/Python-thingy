...
def initialize_blue_party():
    ...
    blue_party[1] = pkmn.CreatePokemon("Pidgeot", 190, 145, 139, 134, 134, 157, blue_movesets["Pidgeot"], "normal", "flying")
    blue_party[2] = pkmn.CreatePokemon("Rhydon", 212, 200, 189, 106, 106, 101, blue_movesets["Rhydon"], "ground", "rock")
    blue_party[3] = pkmn.CreatePokemon("Arcanince", 197, 178, 145, 167, 145, 161, blue_movesets["Arcanine"], "fire", "fire")
    blue_party[4] = pkmn.CreatePokemon("Blastoise", 186, 148, 167, 150, 172, 130, blue_movesets["Blastoise"], "water", "water")
    blue_party[5] = pkmn.CreatePokemon("Exeggutor", 202, 161, 150, 194, 128, 117, blue_movesets["Exeggutor"], "grass", "psychic")
    blue_party[6] = pkmn.CreatePokemon("Alakazam", 162, 112, 106, 205, 150, 189, blue_movesets["Alakazam"], "psychic", "psychic")
...
def initialize_red_party():
    ...
    # Format: name, hp, atk, defense, sp_atk, sp_def, speed, accuracy, moveSet, type1, type2
    red_party[3] = pkmn.CreatePokemon("Charizard", 162, 115, 109, 140, 116, 131, red_movesets["Charizard"], "fire", "flying")
    red_party[2] = pkmn.CreatePokemon("Lapras", 214, 116, 111, 116, 126, 91, red_movesets["Lapras"], "water", "ice")
    red_party[1] = pkmn.CreatePokemon("Jolteon", 200, 96, 91, 141, 126, 162, red_movesets["Jolteon"], "electric", "electric")
    red_party[4] = pkmn.CreatePokemon("Primape", 149, 136, 91, 91, 101, 126, red_movesets["Primape"], "fighting", "fighting")
    red_party[5] = pkmn.CreatePokemon("Nidoking", 165, 123, 107, 116, 106, 116, red_movesets["Nidoking"], "poison", "ground")
    red_party[6] = pkmn.CreatePokemon("Dragonite", 175, 165, 126, 131, 131, 111, red_movesets["Dragonite"], "dragon", "flying")
        ...
        for i in range(1, len(red_party) + 1):
            if red_party[i].getStatus() == "(fainted)":
                if red_party[i].getType1() == red_party[i].getType2():
                    print("[" + str(i) + "]. " + red_party[i].getName() + " (Type: " + red_party[i].getType1().capitalize() + ")(Fainted)")
                else:
                    print("[" + str(i) + "]. " + red_party[i].getName() + " (Type: " + red_party[i].getType1().capitalize() + ", " +
                ...
                if red_party[i].getType1() == red_party[i].getType2():
                    ...
                    print("[" + str(i) + "]. " + red_party[i].getName() + " (HP: " + str(
                        red_party[i].getHP_percentage()) + "%, Type: " + red_party[i].getType1().capitalize() + ", " + red_party[i].getType2().capitalize() + ")")
...
def faint_switch(team):
    ...
    if team == "blue":
        ...
        write(0.03, "CHAMPION Blue is about to use " + blue_party[swap_target].getName() + ". Will Red change Pokémon? (YES/NO)")
        red_change = input()
...
def change_status_stage(attacker, move):
    if (attacker == "blue" and move.getMoveType() == "opponent") or (attacker == "red" and move.getMoveType() == "self"):
        ...
        write(0.03, red_active_pokemon.getName() + "'s " + move.getEffect().capitalize() + " has been changed by " + str(move.getPower()) + " stage(s)!\n")


    elif (attacker == "red" and move.getMoveType() == "opponent") or (attacker == "blue" and move.getMoveType() == "self"):
        if move.getEffect() == "accuracy":
        ...
        write(0.03, "Foe " + blue_active_pokemon.getName().upper() + "'s " + move.getEffect().capitalize() + " has been changed by " + str(move.getPower()) + " stage(s)!\n")
...
def updateHP(damage, target):
    if target == "red":
        ...
        if red_active_pokemon.getHP() > 0:
            write(0.03, red_active_pokemon.getName() + "'s HP is now " + str(red_active_pokemon.getHP_percentage()) + "%\n")
        else:
        ...
        if blue_active_pokemon.getHP() > 0:
            write(0.03, "Foe " + blue_active_pokemon.getName().upper() + "'s HP is now " + str(blue_active_pokemon.getHP_percentage()) + "%\n")
        else:
...
def intro_dialogue():
    ...
    write(0.05, "Blue: While working on my Pokédex, I looked all over for pokémon! Not only that, I assembled teams that would beat")
    write(0.05, "any pokémon type! And now... I am the Pokémon League Champion! Red! Do you know what that means? I'll tell you!")
...
def battle_info():
    if red_active_pokemon.getType1() == red_active_pokemon.getType2():
        print("Red's Pokémon: " + red_active_pokemon.getName() + " (HP: " + str(red_active_pokemon.getHP_percentage()) + "%, Type: "+ red_active_pokemon.getType1().capitalize() + ")")
    else:
        print("Red's Pokémon: " + red_active_pokemon.getName() + " (HP: " + str(red_active_pokemon.getHP_percentage()) + "%, Type: " + red_active_pokemon.getType1().capitalize() + ", " + red_active_pokemon.getType2().capitalize() + ")")
    stage_multiplier_info(red_active_pokemon)

    if blue_active_pokemon.getType1() == blue_active_pokemon.getType2():
        print("Blue's Pokémon: " + blue_active_pokemon.getName() + " (HP: " + str(blue_active_pokemon.getHP_percentage()) + "%, Type: " + blue_active_pokemon.getType1().capitalize() + ")" + "\n")
    else:
        print("Blue's Pokémon: " + blue_active_pokemon.getName() + " (HP: " + str(blue_active_pokemon.getHP_percentage()) + "%, Type: " + blue_active_pokemon.getType1().capitalize() + ", " + blue_active_pokemon.getType2().capitalize() + ")" + "\n")
...
def moveData():
    ...
    for i in range(1, 5):
        if red_active_pokemon.getMoveSet()[i].getAccuracy() is None:
            print("[" + str(i) + "]. " + red_active_pokemon.getMoveSet()[i].getName() + " (Type: " + red_active_pokemon.getMoveSet()[
                i].getType().capitalize() + ", Category: " + str(red_active_pokemon.getMoveSet()[i].getCategory()) + ", Power: " + str(red_active_pokemon.getMoveSet()[i].getPower()) + ", Accuracy: ~~~), Special Effect: " + str(red_active_pokemon.getMoveSet()[i].getEffect().capitalize()) + ")")

        else:
            print("[" + str(i) + "]. " + red_active_pokemon.getMoveSet()[i].getName() + " (Type: " + red_active_pokemon.getMoveSet()[
                i].getType().capitalize() + ", Category: " + str(red_active_pokemon.getMoveSet()[i].getCategory()) + ", Power: " + str(red_active_pokemon.getMoveSet()[i].getPower()) + ", Accuracy: " + str(
                int(red_active_pokemon.getMoveSet()[i].getAccuracy()*100)) + "%, Special Effect: " + str(red_active_pokemon.getMoveSet()[i].getEffect()).capitalize() + ")")


def win_message():
    write(0.05, "Blue: NO! That can't be! You beat me at my best! After all that work to become the League Champ? My reign is over already? It's not fair!")
    write(0.05, "Blue: Why? Why did I lose? I never made any mistakes raising my Pokémon… Darn it! You're the new Pokémon League Champion! Although I don't like to admit it…\n")
